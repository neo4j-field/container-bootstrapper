 ...
 *
 * This file is NOT part of Neo4j...it's hacked by dave.voutila@neo4j.com
 *
...
public class SslPolicyLoader {
    ...
    private KeyAndChain pemKeyAndChain(SslPolicyConfig policyConfig) {
        ...
        // TODO: check logic, had to trim out something temporarily
        if (policyConfig.getScope().isClientOnly()) {
    ...
    private static Collection<X509CRL> getCRLs(File revokedCertificatesDir) {
        ...
        try {
            certificateFactory = CertificateFactory.getInstance(ContainerPkiUtils.CERTIFICATE_TYPE);
    ...
    private static X509Certificate[] loadCertificateChain(AssetUri cert) {
        try {
            ...
            throw new RuntimeException("Failed to load public certificate chain: " + cert, e);
        }
    }
    ...
    private PrivateKey loadPrivateKey(AssetUri key, SecureString privateKeyPassword) {
        ...
        try {
            return ContainerPkiUtils.loadPrivateKey(key.getChannel(), password);
        } catch (Exception e) {
            throw new RuntimeException("Failed to load private key: " + key +
    ...
    private KeyStore createTrustStoreFromPem(File trustedCertificatesDir, ClientAuth clientAuth)
        ...
        for (File trustedCertFile : trustedCertFiles) {
            CertificateFactory certificateFactory = CertificateFactory.getInstance(ContainerPkiUtils.CERTIFICATE_TYPE);
